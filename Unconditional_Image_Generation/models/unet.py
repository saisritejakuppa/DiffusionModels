...
if __name__ == '__main__':
    ...
    #testing attension block
    atten = SelfAttention(4, 16)
    atten_x = torch.randn(1, 4, 16, 16)
    atten_out = atten(atten_x)
    print(atten_out.shape)
    
    # testing doubleconv
    dconv = DoubleConv(3,8)
    dconv_x = torch.randn(1, 3, 16, 16)
    dconv_out = dconv(dconv_x)
    print(dconv_out.shape)
    
    
    #Down conv
    down = Down(16,8)
    down_x = torch.randn(1, 16, 16, 16)
    embedding = torch.randn(1, 256)
    down_out = down(down_x, embedding)
